...
using GameInteraction.Domain.DomainEvents.NotificationDomainEvent;
using GameInteraction.Domain.DomainEvents.RoomDomainEvents;
using GameInteraction.Domain.Enums;
using GameInteraction.Domain.Primitives;
using Message = GameInteraction.Domain.Messages.Message;
using Player = GameInteraction.Domain.Entities.PlayerEntity.Player;

namespace GameInteraction.Domain.Entities.RoomEntity;
...
public sealed partial class Room : AggregateRoot, IHasDomainEvent
{
    ...
    public const ERoomStatus DefaultRoomStatus = ERoomStatus.Waiting;
    ...
    private void AddNewPlayer(Player player) 
    {
        lock (_players)
        {
            ...
            {
                SetRoomStatus(ERoomStatus.Full);
            }
        }
    }
    ...
    public void SetRoomStatus(ERoomStatus status)  // todo make private
    {
        Status = status;
    ...
    private void AddNewGameHistoryMessage(EGameHistoryType type, string currentTime, string message, string? playerName = default)
    {
